...
# repowiki hook start
# Auto-generated by repowiki — do not edit this block
REPOWIKI_BIN="/absolute/path/to/repowiki"
if [ -x "$REPOWIKI_BIN" ]; then
  "$REPOWIKI_BIN" hooks post-commit &
elif command -v repowiki >/dev/null 2>&1; then
  repowiki hooks post-commit &
...
- **Absolute path priority**: Uses the exact binary path that installed the hook for reliability
- **Fallback to PATH**: If absolute path fails, tries `repowiki` from system PATH
- **Existence checks**: Verifies binary is executable (`-x` test) before running
